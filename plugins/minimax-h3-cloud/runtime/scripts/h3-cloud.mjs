...
import { resolveGenerationSettings } from "../src/workflow.mjs";
...
if (command === "generate") {
  ...
  if (referenceImagePaths.length > 0 && (firstFrameImagePath || lastFrameImagePath)) {
    throw new Error("关键帧参数与 --reference-image 不能同时使用");
  }
  const generationMode = referenceImagePaths.length > 0
  ...
  const modeLabel = {
    ...
    "reference-image-to-video": "参考图生视频",
  }[generationMode];
  console.log(
  ...
  try {
    ...
    if (firstFrameImagePath) {
      clientArgs.push("--first-frame-image", firstFrameImagePath);
    }
    if (lastFrameImagePath) {
      clientArgs.push("--last-frame-image", lastFrameImagePath);
    }
    for (const referenceImagePath of referenceImagePaths) {
      clientArgs.push("--reference-image", referenceImagePath);
    }
