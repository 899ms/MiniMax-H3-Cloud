...
function normalizeReferencePrompt(prompt, imageCount) {
  const withoutTags = prompt.replace(/<Picture\s+\d+>/gi, " ").replace(/\s+/g, " ").trim();
  const tags = Array.from(
    { length: imageCount },
    (_, index) => `<Picture ${index + 1}>`,
  ).join(" ");
  return withoutTags ? `${tags} ${withoutTags}` : tags;
}
...
export function configureWorkflow(
  ...
  if (
    ...
    throw new Error("关键帧模式和参考图模式不能同时使用");
  }
    ...
    generationNode.inputs.prompt =
      generationNode.class_type === "MiniMaxH3ReferenceToVideo"
        ? normalizeReferencePrompt(
            prompt,
            Math.max(resolvedReferenceImages.length, 1),
          )
        : prompt;
  }
